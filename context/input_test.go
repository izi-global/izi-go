...
func TestBind(t *testing.T) {
	...
	cases := []struct {
		...
		{"/?human.Nick=diepdt", []testItem{{"human", Human{}, Human{Nick: "diepdt"}}}},
		{"/?human.ID=888&human.Nick=diepdt&human.Ms=true&human[Pwd]=pass", []testItem{{"human", Human{}, Human{ID: 888, Nick: "diepdt", Ms: true, Pwd: "pass"}}}},
		{"/?human[0].ID=888&human[0].Nick=diepdt&human[0].Ms=true&human[0][Pwd]=pass01&human[1].ID=999&human[1].Nick=ysqi&human[1].Ms=On&human[1].Pwd=pass02",
			[]testItem{{"human", []Human{}, []Human{
				{ID: 888, Nick: "diepdt", Ms: true, Pwd: "pass01"},
				{ID: 999, Nick: "ysqi", Ms: true, Pwd: "pass02"},
			}}}},

		{
			"/?id=123&isok=true&ft=1.2&ol[0]=1&ol[1]=2&ul[]=str&ul[]=array&human.Nick=diepdt",
			[]testItem{
				{"id", 0, 123},
				{"isok", false, true},
				{"ft", 0.0, 1.2},
	...
	for _, c := range cases {
		...
		izigoInput := NewInput()
		izigoInput.Context = NewContext()
		izigoInput.Context.Reset(httptest.NewRecorder(), r)

		for _, item := range c.valueGp {
			got := item.empty
			err := izigoInput.Bind(&got, item.field)
			if err != nil {
				t.Fatal(err)
			}
...
func TestSubDomain(t *testing.T) {
	r, _ := http.NewRequest("GET", "http://www.example.com/?id=123&isok=true&ft=1.2&ol[0]=1&ol[1]=2&ul[]=str&ul[]=array&user.Name=diepdt", nil)
	izigoInput := NewInput()
	izigoInput.Context = NewContext()
	izigoInput.Context.Reset(httptest.NewRecorder(), r)

	subdomain := izigoInput.SubDomains()
	if subdomain != "www" {
		t.Fatal("Subdomain parse error, got" + subdomain)
	}

	r, _ = http.NewRequest("GET", "http://localhost/", nil)
	izigoInput.Context.Request = r
	if izigoInput.SubDomains() != "" {
		t.Fatal("Subdomain parse error, should be empty, got " + izigoInput.SubDomains())
	}

	r, _ = http.NewRequest("GET", "http://aa.bb.example.com/", nil)
	izigoInput.Context.Request = r
	if izigoInput.SubDomains() != "aa.bb" {
		t.Fatal("Subdomain parse error, got " + izigoInput.SubDomains())
	}
	...
	*/
	...
	izigoInput.Context.Request = r
	if izigoInput.SubDomains() != "aa.bb.cc.dd" {
		t.Fatal("Subdomain parse error, got " + izigoInput.SubDomains())
	}
}

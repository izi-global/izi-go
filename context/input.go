...
// IZIGoInput operates the http request header, data, cookie and body.
...
// like /?id=123&isok=true&ft=1.2&ol[0]=1&ol[1]=2&ul[]=str&ul[]=array&user.Name=diepdt
// var id int  izigoInput.Bind(&id, "id")  id ==123
// var isok bool  izigoInput.Bind(&isok, "isok")  isok ==true
// var ft float64  izigoInput.Bind(&ft, "ft")  ft ==1.2
// ol := make([]int, 0, 2)  izigoInput.Bind(&ol, "ol")  ol ==[1 2]
// ul := make([]string, 0, 2)  izigoInput.Bind(&ul, "ul")  ul ==[str array]
// user struct{Name}  izigoInput.Bind(&user, "user")  user == {Name:"diepdt"}
func (input *IZIGoInput) Bind(dest interface{}, key string) error {
	value := reflect.ValueOf(dest)
	if value.Kind() != reflect.Ptr {
		return errors.New("izigo: non-pointer passed to Bind: " + key)
	}
	value = value.Elem()
	if !value.CanSet() {
		return errors.New("izigo: non-settable variable passed to Bind: " + key)
	}
	typ := value.Type()
	...
	if !rv.IsValid() {
		return errors.New("izigo: reflect value is empty")
	}
	value.Set(rv)
	return nil
}

func (input *IZIGoInput) bind(key string, typ reflect.Type) reflect.Value {
	if input.Context.Request.Form == nil {
		input.Context.Request.ParseForm()
	}
	rv := reflect.Zero(typ)
...
func (input *IZIGoInput) bindValue(val string, typ reflect.Type) reflect.Value {
	rv := reflect.Zero(typ)
	switch typ.Kind() {
	case reflect.Int, reflect.Int8, reflect.Int16, reflect.Int32, reflect.Int64:
		rv = input.bindInt(val, typ)
...
func (input *IZIGoInput) bindInt(val string, typ reflect.Type) reflect.Value {
...
func (input *IZIGoInput) bindUint(val string, typ reflect.Type) reflect.Value {
...
func (input *IZIGoInput) bindFloat(val string, typ reflect.Type) reflect.Value {
...
func (input *IZIGoInput) bindString(val string, typ reflect.Type) reflect.Value {
	return reflect.ValueOf(val)
}

func (input *IZIGoInput) bindBool(val string, typ reflect.Type) reflect.Value {
...
func (input *IZIGoInput) bindSlice(params *url.Values, key string, typ reflect.Type) reflect.Value {
	maxIndex := -1
	numNoIndex := 0
...
func (input *IZIGoInput) bindStruct(params *url.Values, key string, typ reflect.Type) reflect.Value {
...
func (input *IZIGoInput) bindPoint(key string, typ reflect.Type) reflect.Value {
	return input.bind(key, typ.Elem()).Addr()
}

func (input *IZIGoInput) bindMap(params *url.Values, key string, typ reflect.Type) reflect.Value {
	var (

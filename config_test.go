...
func TestDefaults(t *testing.T) {
	if BConfig.WebConfig.FlashName != "IZIGO_FLASH" {
		t.Errorf("FlashName was not set to default.")
	}

	if BConfig.WebConfig.FlashSeparator != "IZIGOFLASH" {
		t.Errorf("FlashName was not set to default.")
	}
}
...
func TestAssignConfig_01(t *testing.T) {
	_BConfig := &Config{}
	_BConfig.AppName = "izigo_test"
	jcf := &config.JSONConfig{}
	ac, _ := jcf.ParseData([]byte(`{"AppName":"izigo_json"}`))
	assignSingleConfig(_BConfig, ac)
	if _BConfig.AppName != "izigo_json" {
		t.Log(_BConfig)
		t.FailNow()
	}
}
...
func TestAssignConfig_03(t *testing.T) {
	jcf := &config.JSONConfig{}
	ac, _ := jcf.ParseData([]byte(`{"AppName":"izigo"}`))

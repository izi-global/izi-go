...
// IZIAdminApp is the default adminApp used by admin module.
var iziAdminApp *adminApp
...
//			return false
//	 	}
//	 	if strings.HasPrefix(requestPath, "/diepdt") {
//			return false
//	 	}
//	 	return true
// 	}
// 	izigo.FilterMonitorFunc = MyFilterMonitor.
var FilterMonitorFunc func(string, string, time.Duration, string, int) bool

func init() {
	iziAdminApp = &adminApp{
		routers: make(map[string]http.HandlerFunc),
	}
	iziAdminApp.Route("/", adminIndex)
	iziAdminApp.Route("/qps", qpsIndex)
	iziAdminApp.Route("/prof", profIndex)
	iziAdminApp.Route("/healthcheck", healthcheck)
	iziAdminApp.Route("/task", taskStatus)
	iziAdminApp.Route("/listconf", listConf)
...
// ListConf is the http.Handler of displaying all izigo configuration values as key/value pair.
...
func listConf(rw http.ResponseWriter, r *http.Request) {
	...
	case "filter":
		...
		if IZIApp.Handlers.enableFilter {
			...
			for k, fr := range map[int]string{
				...
				if bf := IZIApp.Handlers.filters[k]; len(bf) > 0 {
					filterType = fr
...
func PrintTree() map[string]interface{} {
	...
	for method, t := range IZIApp.Handlers.routers {
...
func printTree(resultList *[][]string, t *Tree) {
	...
	for _, l := range t.leaves {
		if v, ok := l.runObject.(*ControllerInfo); ok {
			if v.routerType == routerTypeIZIGo {
				var result = []string{
					v.pattern,
...
// adminApp is an http.HandlerFunc map used as iziAdminApp.

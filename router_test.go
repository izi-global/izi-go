...
import (
	...
	"github.com/izi-global/izigo/context"
...
func TestUrlFor2(t *testing.T) {
	...
	if handler.URLFor("TestController.GetURL", ":username", "diepdt") != "/v1/diepdt/edit" {
		logs.Info(handler.URLFor("TestController.GetURL"))
		t.Errorf("TestController.List must equal to /v1/diepdt/edit")
	}
...
func TestPostFunc(t *testing.T) {
	r, _ := http.NewRequest("POST", "/diepdt", nil)
	w := httptest.NewRecorder()

	handler := NewControllerRegister()
	handler.Add("/:name", &TestController{})
	handler.ServeHTTP(w, r)
	if w.Body.String() != "diepdt" {
		t.Errorf("post func should diepdt")
	}
}
...
func TestManyRoute(t *testing.T) {
	...
	r, _ := http.NewRequest("GET", "/izigo32-12.html", nil)
	w := httptest.NewRecorder()

	handler := NewControllerRegister()
	handler.Add("/izigo:id([0-9]+)-:page([0-9]+).html", &TestController{}, "get:GetManyRouter")
	handler.ServeHTTP(w, r)
...
func TestEmptyResponse(t *testing.T) {
	...
	r, _ := http.NewRequest("GET", "/izigo-empty.html", nil)
	w := httptest.NewRecorder()

	handler := NewControllerRegister()
	handler.Add("/izigo-empty.html", &TestController{}, "get:GetEmptyBody")
	handler.ServeHTTP(w, r)
...
func izigoFilterFunc(ctx *context.Context) {
...
func TestRouterFunc(t *testing.T) {
	...
	mux.Get("/action", izigoFilterFunc)
	mux.Post("/action", izigoFilterFunc)
...
func TestParamResetFilter(t *testing.T) {
	...
	route := "/izigo/*" // splat
	path := "/izigo/routes/routes"

	mux := NewControllerRegister()

	mux.InsertFilter("*", BeforeExec, izigoResetParams, true, true)

	mux.Get(route, izigoHandleResetParams)

	rw, r := testRequest("GET", path)
	mux.ServeHTTP(rw, r)

	// The two functions, `izigoResetParams` and `izigoHandleResetParams` add
...
func TestFilterBeforeRouter(t *testing.T) {
	...
	mux := NewControllerRegister()
	mux.InsertFilter(url, BeforeRouter, izigoBeforeRouter1)

	mux.Get(url, izigoFilterFunc)
...
func TestFilterBeforeExec(t *testing.T) {
	...
	mux := NewControllerRegister()
	mux.InsertFilter(url, BeforeRouter, izigoFilterNoOutput)
	mux.InsertFilter(url, BeforeExec, izigoBeforeExec1)
...
func TestFilterAfterExec(t *testing.T) {
	...
	mux := NewControllerRegister()
	mux.InsertFilter(url, BeforeRouter, izigoFilterNoOutput)
	mux.InsertFilter(url, BeforeExec, izigoFilterNoOutput)
	mux.InsertFilter(url, AfterExec, izigoAfterExec1, false)
...
func TestFilterFinishRouter(t *testing.T) {
	...
	mux.InsertFilter(url, BeforeRouter, izigoFilterNoOutput)
	mux.InsertFilter(url, BeforeExec, izigoFilterNoOutput)
	mux.InsertFilter(url, AfterExec, izigoFilterNoOutput)
	mux.InsertFilter(url, FinishRouter, izigoFinishRouter1)
...
func TestFilterFinishRouterMultiFirstOnly(t *testing.T) {
	...
	mux := NewControllerRegister()
	mux.InsertFilter(url, FinishRouter, izigoFinishRouter1, false)
	mux.InsertFilter(url, FinishRouter, izigoFinishRouter2)
...
func TestFilterFinishRouterMulti(t *testing.T) {
	...
	mux := NewControllerRegister()
	mux.InsertFilter(url, FinishRouter, izigoFinishRouter1, false)
	mux.InsertFilter(url, FinishRouter, izigoFinishRouter2, false)
...
func izigoFilterNoOutput(ctx *context.Context) {
}

func izigoBeforeRouter1(ctx *context.Context) {
	ctx.WriteString("|BeforeRouter1")
}

func izigoBeforeRouter2(ctx *context.Context) {
	ctx.WriteString("|BeforeRouter2")
}

func izigoBeforeExec1(ctx *context.Context) {
	ctx.WriteString("|BeforeExec1")
}

func izigoBeforeExec2(ctx *context.Context) {
	ctx.WriteString("|BeforeExec2")
}

func izigoAfterExec1(ctx *context.Context) {
	ctx.WriteString("|AfterExec1")
}

func izigoAfterExec2(ctx *context.Context) {
	ctx.WriteString("|AfterExec2")
}

func izigoFinishRouter1(ctx *context.Context) {
	ctx.WriteString("|FinishRouter1")
}

func izigoFinishRouter2(ctx *context.Context) {
	ctx.WriteString("|FinishRouter2")
}

func izigoResetParams(ctx *context.Context) {
	ctx.ResponseWriter.Header().Set("splat", ctx.Input.Param(":splat"))
}

func izigoHandleResetParams(ctx *context.Context) {
	ctx.ResponseWriter.Header().Set("splat", ctx.Input.Param(":splat"))
}

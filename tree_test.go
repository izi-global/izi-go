...
func TestTreeRouters(t *testing.T) {
	for _, r := range routers {
		tr := NewTree()
		tr.AddRouter(r.url, "diepdt")
		ctx := context.NewContext()
		obj := tr.Match(r.requesturl, ctx)
		if obj == nil || obj.(string) != "diepdt" {
...
func TestAddTree(t *testing.T) {
	tr := NewTree()
	tr.AddRouter("/shop/:id/account", "diepdt")
	tr.AddRouter("/shop/:sd/ttt_:id(.+)_:page(.+).html", "diepdt")
	t1 := NewTree()
	t1.AddTree("/v1/zl", tr)
	ctx := context.NewContext()
...
func TestAddTree3(t *testing.T) {
	tr := NewTree()
	tr.AddRouter("/create", "diepdt")
	tr.AddRouter("/shop/:sd/account", "diepdt")
	t3 := NewTree()

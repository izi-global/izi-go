...
// Handler izigo filter handler for serve captcha image
...
func NewWithFilter(urlPrefix string, store cache.Cache) *Captcha {
	...
	izigo.InsertFilter(cpt.URLPrefix+"*", izigo.BeforeRouter, cpt.Handler)

	// add to template func map
	izigo.AddFuncMap("create_captcha", cpt.CreateCaptchaHTML)

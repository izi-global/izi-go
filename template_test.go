...
<html>
  <head>
    <title>izigo welcome template</title>
  </head>
  <body>
{{template "block"}}
{{template "header"}}
...
func TestTemplate(t *testing.T) {
	...
	iziTemplates := iziViewPathTemplates[dir]
	if len(iziTemplates) != 3 {
		t.Fatalf("should be 3 but got %v", len(iziTemplates))
	}
	if err := iziTemplates["index.tpl"].ExecuteTemplate(os.Stdout, "index.tpl", nil); err != nil {
		t.Fatal(err)
	}
...
func TestRelativeTemplate(t *testing.T) {
	...
	iziTemplates := iziViewPathTemplates[dir]
	if err := iziTemplates["easyui/rbac/user.tpl"].ExecuteTemplate(os.Stdout, "easyui/rbac/user.tpl", nil); err != nil {
		t.Fatal(err)
	}
	for _, name := range files {
		os.RemoveAll(filepath.Join(dir, name))
...
func TestTemplateLayout(t *testing.T) {
	...
	iziTemplates := iziViewPathTemplates[dir]
	if len(iziTemplates) != 2 {
		t.Fatalf("should be 2 but got %v", len(iziTemplates))
	}
	out := bytes.NewBufferString("")
	if err := iziTemplates["add.tpl"].ExecuteTemplate(out, "add.tpl", map[string]string{"Title": "Hello", "SomeVar": "val"}); err != nil {
		t.Fatal(err)
	}

...
func TestCRUD(t *testing.T) {
	...
	err = dORM.Read(u)
	throwFailNow(t, err)
	throwFail(t, AssertIs(u.UserName, "diepdt"))
	throwFail(t, AssertIs(u.Profile.ID, profile.ID))

	u = &User{UserName: "diepdt", Password: "pass"}
...
func TestInsertTestData(t *testing.T) {
	...
	user = NewUser()
	user.UserName = "diepdt"
	user.Email = "dotiendiep@gmail.com"
...
func TestCustomField(t *testing.T) {
	...
	throwFailNow(t, AssertIs(user.Extra.Name, "izigo"))
...
func TestOrderBy(t *testing.T) {
	...
	num, err = qs.OrderBy("-profile__age").Filter("user_name", "diepdt").Count()
	throwFail(t, err)
	throwFail(t, AssertIs(num, 1))
}
...
func TestAll(t *testing.T) {
	...
	throwFail(t, AssertIs(users[1].UserName, "diepdt"))
	...
	throwFailNow(t, AssertIs(users2[1].UserName, "diepdt"))
...
func TestValuesFlat(t *testing.T) {
	...
	if num == 3 {
		...
		throwFail(t, AssertIs(list[1], "diepdt"))
...
func TestRelatedSel(t *testing.T) {
	...
	throwFailNow(t, AssertIs(posts[1].User.UserName, "diepdt"))
	throwFailNow(t, AssertIs(posts[2].User.UserName, "diepdt"))
...
func TestReverseQuery(t *testing.T) {
	...
	profile = Profile{}
	err = dORM.QueryTable("user_profile").Filter("User__UserName", "diepdt").One(&profile)
	throwFailNow(t, err)
	throwFailNow(t, AssertIs(profile.Age, 30))
	...
	throwFailNow(t, err)
	throwFailNow(t, AssertIs(user.UserName, "diepdt"))

	user = User{}
	err = dORM.QueryTable("user").Filter("Posts__User__UserName", "diepdt").Limit(1).One(&user)
	throwFailNow(t, err)
	throwFailNow(t, AssertIs(user.UserName, "diepdt"))

	user = User{}
	err = dORM.QueryTable("user").Filter("Posts__User__UserName", "diepdt").RelatedSel().Limit(1).One(&user)
	throwFailNow(t, err)
	throwFailNow(t, AssertIs(user.UserName, "diepdt"))
	...
	num, err = dORM.QueryTable("tag").Filter("Posts__Post__Title", "Introduction").
		Filter("BestPost__User__UserName", "diepdt").All(&tags)
	throwFailNow(t, err)
	throwFailNow(t, AssertIs(num, 1))
	throwFailNow(t, AssertIs(tags[0].Name, "golang"))
	...
	tags = []*Tag{}
	num, err = dORM.QueryTable("tag").Filter("Posts__Post__Title", "Introduction").
		Filter("BestPost__User__UserName", "diepdt").RelatedSel().All(&tags)
	throwFailNow(t, err)
	throwFailNow(t, AssertIs(num, 1))
	throwFailNow(t, AssertIs(tags[0].Name, "golang"))
	...
	throwFailNow(t, AssertIs(tags[0].BestPost.User.UserName, "diepdt"))
}
...
func TestLoadRelated(t *testing.T) {
	...
	throwFailNow(t, AssertIs(num, 2))
	throwFailNow(t, AssertIs(len(user.Posts), 2))
	throwFailNow(t, AssertIs(user.Posts[0].User.UserName, "diepdt"))
	...
	throwFailNow(t, AssertIs(profile.User.UserName, "diepdt"))
	...
	throwFailNow(t, AssertIs(post.User.UserName, "diepdt"))
	...
	throwFailNow(t, AssertIs(post.Tags[0].BestPost.User.UserName, "diepdt"))
...
func TestQueryRows(t *testing.T) {
	...
	throwFailNow(t, AssertIs(usernames[1], "diepdt"))
	...
	throwFailNow(t, AssertIs(l[1].UserName, "diepdt"))
...
func TestInsertOrUpdate(t *testing.T) {
	...
	if err != nil {
		fmt.Println(err)
		if err.Error() == "postgres version must 9.5 or higher" || err.Error() == "`sqlite3` nonsupport InsertOrUpdate in izigo" {
		} else {
			throwFailNow(t, err)
		}
	} else {
